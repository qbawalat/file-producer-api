...
public class ParsedFileMocker {
    ...
    public static ParsedFile mockParsedFileWithOneRecord(String fileName, String... entries) {
        return mockParsedFile(fileName, mockParsedRecords(entries));
    }

    public static ParsedRecord mockParsedRecords(String... entries) {
        ...
        for (int iterator = 0; iterator < entries.length; iterator += 2) {
            parsedRecord.put(entries[iterator], entries[iterator + 1]);
        }
        return parsedRecord;
    }
}
